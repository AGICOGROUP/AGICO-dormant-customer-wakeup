...
**⚠️ 铁律：从第1页开始逐页提取。当前页有合格客户就提取完直接停止，不翻下一页。当前页0个合格才翻下一页。**

| 条件 | 动作 |
|------|------|
| `qualifiedCount > 0` | **停止翻页**。提取到的客户全部保存，直接进入0.5保存 → 0.6输出 → 阶段一 |
| `qualifiedCount = 0` | 翻到下一页（PAGE_NUM+1），重新执行0.3脚本 |
| `page * 100 >= totalCustomerCount` | 到末页仍无合格客户，告知用户"未找到符合条件的沉睡客户" |

> **示例**：第1页0个合格→翻第2页→第2页2个合格→提取流程结束，先处理这2个客户，处理完了再翻第3页找更多。
>
...
- 当前页提取到的客户全部处理完后 → 提示用户"本批已全部完成，是否继续翻页找下一批？"
- 用户确认 → 从下一页（PAGE_NUM+1）继续执行0.3脚本，同样遵守"有则停"铁律
- 用户拒绝 → 保存当前进度，下次继续
...
当需要向文件写入超长内容（HTML报告、大型脚本、邮件正文拼接等）时，必须遵守以下规则以防 `write` 工具截断导致数据丢失：

### 触发条件
- 待写入内容超过 2000 字符
- 包含复杂嵌套结构（HTML表格、多行代码）
- 生成 HTML/PDF 案例报告
- 邮件正文拼接
...
| 场景 | 禁止 | 推荐 |
|------|------|------|
| 长HTML报告 | `write` 全量覆盖 | `write` 写骨架 → `edit` 分段填充 |
| 大段脚本 | `write` 一次性写入 | `write` 写空框架 → `edit` 逐段替换 |
| 邮件正文拼接 | `append: true` 追加超长文本 | `edit` 替换占位符 |
| 任何 >2000字符内容 | 直接 `write` | 先 `write` 小块测试 → `edit` 分批写入 |

### 操作流程

1. **写骨架**：先用 `write` 写入一个包含占位符的最小文件（如 `<!-- REPORT_BODY -->`）
2. **分段填充**：用 `edit` 将占位符替换为实际内容，每次替换不超过 2000 字符
3. **校验完整性**：`read` 读取修改区域，确认内容完整无截断
4. **锚点唯一性**：`edit` 的 `old_string` 必须具有唯一性，用 `grep` 确认

### 异常处理

| 情况 | 处理 |
|------|------|
| `write` 参数被截断 | 立即改用 `edit` 分段写入 |
| `edit` 找不到 `old_string` | `read` 重新获取最新上下文，更新锚点 |
| 内容仍被截断 | 拆成更小的段（500字符一段），逐段 `edit` |
